...
export default function ManageBooking() {
  ...
  const approveBooking = async (bookingId: string) => {
    if (!bookingId) return;
    const res = await updateBooking({ bookingId, payload: { status: "approved" } });
    res?.data?.success ? toast.success("Booking Approved!") : toast.error("Failed to approve booking");
  };
  ...
  const cancelBookingIntoDB = async (bookingId: string, carId: string) => {
    ...
    res?.data?.success ? toast.success("Booking Cancelled!") : toast.error("Failed to cancel booking");
  };

  const confirmReturnBooking = async (bookingId: string) => {
    if (!bookingId) return;
    const res = await returnCar({ bookingId });
    res?.data?.success ? toast.success("Car Returned Successfully") : toast.error("Failed to process return");
  };
  ...
  return (
    <section className="max-w-[1500px] mx-auto px-4 py-12">
      <div className="text-center mb-12">
        ...
        <div className="mt-3 text-gray-600 space-y-1 text-sm" style={{ fontFamily: "'Poppins', sans-serif" }}>
                ...
                return (
                  ...
                  >
                    {booking.isReturnProcess && (
                      <td colSpan={9} className="absolute inset-0 flex justify-center items-center bg-black/50 z-10">
                        <button
                    ...
                    <td className="px-4 py-3">{car?.name || "Unknown"}</td>
                    ...
                    <td className="px-4 py-3">
                      {["cancelled", "completed"].includes(booking.status ?? "") ? (
                        ...
                        <button
                          onClick={() => cancelBookingIntoDB(bookingId, carId)}
