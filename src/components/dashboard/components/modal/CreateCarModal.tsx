...
export default function CreateCarModal({ open, setOpen }: TModalProps) {
    ...
    async (data: any) => {
      ...
      try {
        ...
        if (response?.success) {
          setOpen(false);
          reset();
          toast.success("New car created successfully", { duration: 2000 });
        }
      } catch (error) {
  ...
  return (
    <section
      ...
      role="dialog"
      aria-labelledby="create-car-modal-title"
      aria-modal="true"
    >
      <form
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex gap-3 sm:gap-4">
              <button
                type="submit"
                className="flex-1 bg-gradient-to-r from-yellow-500 to-red-600 text-white py-2 rounded-lg font-semibold hover:from-yellow-600 hover:to-red-700 focus:ring-2 focus:ring-yellow-500 focus:outline-none transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                style={{ fontFamily: "'Poppins', sans-serif" }}
                disabled={isLoading}
                aria-label="Create new car"
              >
                Create
              </button>
              <button
